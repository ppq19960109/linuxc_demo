...
int HilinkOtaStartProcess(int type)
{
    ...
    {
        printf("HilinkOtaStartProcess UPDATE_TYPE_MANUAL:%d\n", type);
    }
    else
    {
        printf("HilinkOtaStartProcess UPDATE_TYPE_AUTO:%d\n", type);
    }
    return RETURN_OK;
}
...
{
    /* 厂商实现此接口 */
    printf("HilinkOtaEndProcess status:%d\n", status);
    if (status != 100)
        return -1;
    return RETURN_OK;
}
...
int HilinkGetRebootFlag(void)
{
    ...
    printf("HilinkGetRebootFlag time:%s,hour:%d\n", asctime(localtm), localtm->tm_hour);
...
{
    /* 厂商实现此接口 */
    strcpy(filePath, HILINK_OTA_PATH);
    printf("HILINK_GetUpdateFilePath:%s,len:%d\n", filePath, len);
    return RETURN_OK;
}
...
{
    /* 厂商实现此接口 */
    return HILINK_OTA_MAX_FILE_SIZE;
}
...
{
    /* 厂商实现此接口 */
    printf("HILINK_StartSoftwareIntall\n");
    system("chmod 777 "HILINK_OTA_PATH);
    system("cd /tmp;"HILINK_OTA_PATH" &");
    return RETURN_OK;
}
