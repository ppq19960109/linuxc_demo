...
static irqreturn_t key0_handler(int irq, void *dev_id)
{
	...
	// mod_timer(&dev->timer, jiffies + msecs_to_jiffies(5)); /* 10ms定时 */
...
void testtasklet_func(unsigned long data)
{
	...
	//input_event(dev->inputdev, EV_KEY, keydesc->value, 0);
	input_report_key(dev->inputdev, keydesc->value, value);
	input_sync(dev->inputdev);
...
static int keyio_init(void)
{
	...
	keyinputdev.nd = of_find_node_by_path("/gpio-rkkey");
	...
	for (i = 0; i < KEY_NUM; i++)
	{
		keyinputdev.irqkeydesc[i].gpio = of_get_named_gpio(keyinputdev.nd, "gpio", i);
	...
	for (i = 0; i < KEY_NUM; i++)
	{
		...
		sprintf(keyinputdev.irqkeydesc[i].name, "RKKEY%d", i);							   /* 组合名字 */
...
static int key_probe(struct platform_device *dev)
{
	...
	tasklet_init(&keyinputdev.testtasklet, testtasklet_func, (unsigned long)&keyinputdev);
	return 0;
}
