...
#define IMX6UIRQ_NAME "rkasync" /* 名字 		*/
...
static irqreturn_t key0_handler(int irq, void *dev_id)
{
	...
	dev->curkeynum = 0;
	tasklet_schedule(&dev->testtasklet);
...
void testtasklet_func(unsigned long data)
{
	...
	{ /* 一次完整的按键过程 */
		if (time_after(jiffies, dev->press_time + HZ * 5))
		{
...
static int keyio_init(void)
{
	...
	imx6uirq.nd = of_find_node_by_path("/gpio-rkkey");
	if (imx6uirq.nd == NULL)
	{
	...
	for (i = 0; i < KEY_NUM; i++)
	{
		imx6uirq.irqkeydesc[i].gpio = of_get_named_gpio(imx6uirq.nd, "gpio", i);
	...
	for (i = 0; i < KEY_NUM; i++)
	{
		...
		sprintf(imx6uirq.irqkeydesc[i].name, "RKKEY%d", i);							 /* 组合名字 */
...
static int imx6uirq_probe(struct platform_device *dev)
{
	/* 1、构建设备号 */
	if (imx6uirq.major)
	{
...
static int imx6uirq_remove(struct platform_device *dev)
{
	int i = 0;
	tasklet_kill(&imx6uirq.testtasklet);
	/* 释放中断 */
	for (i = 0; i < KEY_NUM; i++)
	{
		free_irq(imx6uirq.irqkeydesc[i].irqnum, &imx6uirq);
...
static struct platform_driver imx6uirq_driver = {
	.driver = {
		.name = "keyinput",					 /* 驱动名字，用于和设备匹配 */
		.of_match_table = imx6uirq_of_match, /* 设备树匹配表 */
	},
