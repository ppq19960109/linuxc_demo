...
def up(inValue, valueLen):
    ...
    # 场景按键上报
        ...
        print(inValue)
        out1 = 'SceName_' + str(int(inValue[12:16], 16)+1)
        len1 = len(out1)
        str16 = inValue[16:]
        strBin = binascii.unhexlify(str16)
        str8 = bytes.decode(strBin, encoding="utf-16BE")
        str8 = str8.encode('utf-8')
        out2 = ''
        for item in range(len(str8)):
            out2 = out2+hex(str8[item])[2:]
        len2 = len(out2)
    # 场景按键上报
    elif inValue[4:6] == '77':
        out1 = 'KeyFobValue'
...
def down(inKey, keyLen, inValue, valueLen):
    ...
    # 场景18图片
        ...
        len2 = 16
    # 场景名称
    elif inKey[0:7] == 'SceName':
        butten=str(hex(int(inKey[8:])-1))[2:]
        print("buten:",butten)
        if len(butten)==1:
            butten='000'+butten
        elif len(butten)==2:
        ...
        strBin = binascii.unhexlify(inValue)
        strBin8 = strBin.decode('utf-8')
        nameStr16 = strBin8.encode('utf-16BE')
        nameBin16=binascii.hexlify(nameStr16)
        name=nameBin16.decode() + '0'*(16-len(nameBin16))
        out2 = '75' + '00' + '00' + '0a' + butten + name
        len2 = 28
    # 场景按键
...
if __name__ == '__main__':
    ...
    str1 = 'e6b497e6bcb1'
    [out1,len1,out2,len2] = up(str1,len(str1))

    [out2,len2]=down("SceName_1", "9", "e585a8e585b3", "12")
    print(out2)
    print(len2)