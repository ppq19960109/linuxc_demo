...
#define USE_EPOLL 0
#if USE_EPOLL
#define EPOLL_SEND_LEN 4096 //4k
    char sendBuf[EPOLL_SEND_LEN + 1];
    int sendSize;
    int epollfd;
#endif
    int socketfd;
...
#define MAX_EVENTS 10
        ...
        int nfds, n;
        pdata->epollfd = epoll_create1(EPOLL_CLOEXEC);
        if (pdata->epollfd == -1)
        {
        ...
        if (epoll_ctl(pdata->epollfd, EPOLL_CTL_ADD, pdata->socketfd, &ev) == -1)
        {
        ...
        {
#if USE_EPOLL
            nfds = epoll_wait(pdata->epollfd, events, MAX_EVENTS, 0);
            if (nfds == -1)
            {
...
//----------------------------------------
                    ...
                    if (events[n].events & EPOLLOUT)
                    {
                        ...
                        printf("-------socket fd epoll out----------\n");

                        if (Send(nativeClientNet.socketfd, pdata->sendBuf, pdata->sendSize, 0) < 0)
                            continue;
                        ...
                        ev.events = EPOLLIN;
                        ev.data.fd = pdata->socketfd;
                        if (epoll_ctl(pdata->epollfd, EPOLL_CTL_MOD, pdata->socketfd, &ev) == -1)
                        {
                            perror("epoll_ctl: listen_sock");
                        }
...
static int nativeClientWrite(void *recv, unsigned int dataLen)
{
    ...
    ev.data.fd = nativeClientNet.socketfd;
    if (epoll_ctl(nativeClientNet.epollfd, EPOLL_CTL_MOD, nativeClientNet.socketfd, &ev) == -1)
    {
        perror("epoll_ctl: listen_sock");
        ret = -1;
        goto fail;
    }
    memcpy(nativeClientNet.sendBuf, recv, dataLen);
    nativeClientNet.sendSize = dataLen;
fail:
#else
    ret = Send(nativeClientNet.socketfd, recv, dataLen, 0);
#endif
    pthread_mutex_unlock(&nativeClientNet.mutex);
...
#if USE_EPOLL
    if (nativeClientNet.epollfd != 0)
    {
        Close(nativeClientNet.epollfd);
        nativeClientNet.epollfd = 0;
    }
#endif
    if (nativeClientNet.socketfd != 0)
    {
