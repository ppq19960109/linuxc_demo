...
static void demo_dm_recv_property_set(void *dm_handle, const aiot_dm_recv_t *recv, void *userdata)
{
    ...
    */
    cloud_set_json = malloc(recv->data.property_set.params_len + 1);
    strncpy(cloud_set_json, recv->data.property_set.params, recv->data.property_set.params_len);
    cloud_set_json[recv->data.property_set.params_len] = 0;
...
int link_model_start()
{
    ...
    {
        res = link_dynregmq_start(mqtt_host, product_key, product_secret, device_name, device_secret);
        printf("link_dynregmq_start:%d\n", res);
        if (res < 0)
            return -1;
