...
#define DB_NAME "scene_ali.db"
#define TABLE_NAME "scene_ali"
typedef struct
{
    sqlite3 *db;
...
static const char *sqlSelectBySceneId = "SELECT * FROM " TABLE_NAME " WHERE SCENEID = ?;";
static const char *sqlInsert = "INSERT OR REPLACE INTO " TABLE_NAME " VALUES (NULL,?,?);";
static const char *sqlDeleteBySceneId = "DELETE FROM " TABLE_NAME " WHERE SCENEID = ?;";
...
int deleteDatabse(const char *devId)
{
    ...
    sqlite3_stmt *pstmt;
    const char *pzTail;
    int rc = sqlite3_prepare_v2(sqlHandle.db, sqlDeleteBySceneId, -1, &pstmt, &pzTail);
    if (SQLITE_OK != rc)
    {
...
{
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(sqlHandle.db, sqlSelectBySceneId, -1, &stmt, NULL);
    if (SQLITE_OK != rc)
    {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (addFunc != NULL)
            addFunc(src_scene_id, (const char *)devid, (const char *)modelid);
    }
