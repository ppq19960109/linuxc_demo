...
char * __nopoll_conn_get_client_init (noPollConn * conn, noPollConnOpts * opts)
{
    ...
    conn->handshake->expected_accept = nopoll_strdup ((char *)key);
    ...
    return nopoll_strdup_printf ("GET %s HTTP/1.1"
                                 ...
                                 (opts && opts->exttunnel_headers) ? opts->exttunnel_headers : "");
}
                                       ...
                                       const char      * origin)
{
    ...
    if(ctx->read_handler != NULL && ctx->write_handler != NULL)
    {
        conn->receive = ctx->read_handler;
        conn->send    = ctx->write_handler;
    }
                              ...
                              const char * origin)
{
    ...
    return __nopoll_conn_new_common (ctx, NULL, NOPOLL_TRANSPORT_IPV4,
...
{
    /* call common implementation */
    return __nopoll_conn_new_common (ctx, NULL, NOPOLL_TRANSPORT_IPV6,
                                     NOPOLL_INVALID_SOCKET,
                                     host_ip, host_port, host_name,
                                     get_url, protocols, origin);
}
...
{
    /* call common implementation */
    return __nopoll_conn_new_common (ctx, opts, NOPOLL_TRANSPORT_IPV4,
                                     NOPOLL_INVALID_SOCKET,
                                     host_ip, host_port, host_name,
                                     get_url, protocols, origin);
}
 ...
 * @param ctx The noPoll context to which this new connection will be associated.
 *
 * @param options Optional configuration object. See \ref nopoll_conn_opts_new and \ref  (for example).
 *
...
{
    /* call common implementation */
    return __nopoll_conn_new_common (ctx, options, NOPOLL_TRANSPORT_IPV4,
...
void          nopoll_conn_shutdown (noPollConn * conn)
{
...
#if defined(SHOW_DEBUG_LOG)
    if (conn->role == NOPOLL_ROLE_UNKNOWN)
        role = "unknown";
    ...
    else if (conn->session != NOPOLL_INVALID_SOCKET) {
...
nopoll_readline_again:
        if (( rc = conn->receive (conn, &c, 1)) == 1) {
            *ptr++ = c;
            ...
            if (nopoll_conn_is_ok (conn) && errno == 0) {
...
noPollMsg   * nopoll_conn_get_msg (noPollConn * conn)
{
    ...
    /*
    	nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG,
    		    "=== START: conn-id=%d (errno=%d, session: %d, conn->handshake_ok: %d) ===",
    		    conn->id, errno, conn->session, conn->handshake_ok);
    	*/
    ...
    if (conn->previous_msg) {
        nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG, "Reading bytes (previously read %l) from a previous unfinished frame (pending: %d) over conn-id=%d",
        ...
        if (conn->previous_msg->payload_size > 0) {
            ...
            if (msg->is_masked) {
                nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG, "Reusing mask value = %d", nopoll_get_32bit (msg->mask));
            ...
            nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG, "reusing noPollMsg reference (%p) since last payload read was 0, remaining: %l", msg,
                        msg->payload_size);
        }
...

{
    ...
    while (nopoll_true) {
        ...
        if (sleep_in_header == 0) {
            ...
            if (bytes_written == header_size) {
                ...
                nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG, "Rest of content written %d (header size: %d, length: %l)",
    ...
    */
    ...
    if (conn->pending_write_bytes > 0) {
        ...
        nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG, "Stored %d bytes starting from %d out of %l bytes (header size: %d)",
...
void *nopoll_conn_get_userdata (noPollConn * conn)
{
    if(NULL == conn || NULL == conn->ctx)
        return NULL;
