...
void cloud_add_device_attr(const int index, dev_hilink_t *out, const char *sn)
{
    ...
    HilinkSyncBrgDevStatus(sn, DEV_ONLINE);
}
void cloud_add_device(const int index, dev_hilink_t **out, const char *sn, struct list_head *cloudNode)
{
...
void cloud_update_device_int_array(cJSON *root, char **key, char *value, char len, dev_hilink_t *out, int pos)
{
    int i;
    for (i = 0; i < len; ++i)
    {
...
void cloud_update_device_str(cJSON *root, char *key, char *value, dev_hilink_t *out, int pos)
{
...
int local_tohilink(dev_data_t *src,const int index, struct list_head *cloudNode)
{
    ...
    if (out == NULL)
    {
        cloud_add_device(index, &out, src->DeviceId, cloudNode);
    ...
    case 5: //2路智能开关模块（HY0122，型号IHC1239）
    {
        ...
        cloud_update_device_int(root, STR_MODE, dev_sub->LedEnable, out, pos++);
        //Switch_All
        cloud_update_device_int(root, STR_ON, dev_sub->Switch_All, out, pos++);
    }
    break;
    ...
    case 7: //门窗传感器
    {
        ...
        //doorEvent
        cloud_update_device_int(root, STR_ON, dev_sub->ContactAlarm, out, pos++);
    ...
    case 8: //U2/天际系列：智镜/全面屏/触控屏（HY0134）
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            if (out_sub[j] == NULL)
            {
                cloud_add_device(index + j + 1, &out_sub[j], sn, cloudNode);
