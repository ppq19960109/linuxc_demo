...
static HI_S32 SharpenCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_SHARPEN_S *pstSharpen)
{
    ...
    for (i = 0; i < ISP_AUTO_ISO_STRENGTH_NUM; i++)
    {
        ...
        if (pstSharpen->stAuto.au8RGain[i] > HI_ISP_SHARPEN_RGAIN )
        {
            ISP_TRACE(HI_DBG_ERR, "err au8RGain[%d]! Value range:[0, %d]\n", i, HI_ISP_SHARPEN_RGAIN);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

		if (pstSharpen->stAuto.au8GGain[i] > HI_ISP_SHARPEN_GGAIN )
        {
            ISP_TRACE(HI_DBG_ERR, "err au8GGain[%d]! Value range: [0, %d]\n", i, HI_ISP_SHARPEN_GGAIN);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

        if (pstSharpen->stAuto.au8BGain[i] > HI_ISP_SHARPEN_BGAIN )
        {
            ISP_TRACE(HI_DBG_ERR, "err au8BGain[%d]! Value range:[0, %d]\n", i, HI_ISP_SHARPEN_BGAIN);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
    ...
    if (pstSharpen->stManual.u8RGain > HI_ISP_SHARPEN_RGAIN )
    {
        ISP_TRACE(HI_DBG_ERR, "err u8RGain! Value range:[0, %d]\n", HI_ISP_SHARPEN_RGAIN);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
	if (pstSharpen->stManual.u8GGain > HI_ISP_SHARPEN_GGAIN )
    {
        ISP_TRACE(HI_DBG_ERR, "Invalid  u8GGain is [0, %d]\n", HI_ISP_SHARPEN_GGAIN);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
    if (pstSharpen->stManual.u8BGain > HI_ISP_SHARPEN_BGAIN)
    {
        ISP_TRACE(HI_DBG_ERR, "err u8BGain! Value range:[0, %d]\n", HI_ISP_SHARPEN_BGAIN);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
...
static void SharpenCheckStaticReg(ISP_SHARPEN_STATIC_REG_CFG_S *pstShrpReg)
{
    ...
    // DitherMode
    pstShrpReg->u8ditherMode  = SHARPEN_CLIP3( 0, 0x2, pstShrpReg->u8ditherMode  );

    // Direction judgement stbEnChrSad
    pstShrpReg->u8dirVarScale = SHARPEN_CLIP3(0, 0xC,  pstShrpReg->u8dirVarScale);       // U4.0
    pstShrpReg->u8gainThdSelD   = SHARPEN_CLIP3(0, 0x2, pstShrpReg->u8gainThdSelD);    // U2.0
    ...
    // Shoot control
    pstShrpReg->u16oMaxChg    = SHARPEN_CLIP3( 0, 0x3FF, pstShrpReg->u16oMaxChg  ); // U8.0
    pstShrpReg->u16uMaxChg    = SHARPEN_CLIP3( 0, 0x3FF, pstShrpReg->u16uMaxChg  ); // U8.0
    // Control shoot based on variance
    pstShrpReg->u8shtVarSft       = SHARPEN_CLIP3(0, 0x7,  pstShrpReg->u8shtVarSft);         // U3.0
    pstShrpReg->u8shtVarDiffWgt0 = SHARPEN_CLIP3(0, 0x7F, pstShrpReg->u8shtVarDiffWgt0);   // U0.7
    pstShrpReg->u8shtVarWgt1     = SHARPEN_CLIP3(0, 0x7F, pstShrpReg->u8shtVarWgt1);   // U0.7
    ...
    pstShrpReg->u8chrRVarSft   = SHARPEN_CLIP3(0, 0xC, pstShrpReg->u8chrRVarSft);    // U4.0, [0,12]
    pstShrpReg->u8chrBVarSft   = SHARPEN_CLIP3(0, 0xC, pstShrpReg->u8chrBVarSft);    // U4.0, [0,12]
    for ( i = 0; i < 3; i++ )
    {
        pstShrpReg->u8chrRSft[i]  =  SHARPEN_CLIP3(0, 0x7, pstShrpReg->u8chrRSft[i]);   // U0.3 [0,7]
        pstShrpReg->u8chrBSft[i]  =  SHARPEN_CLIP3(0, 0x7, pstShrpReg->u8chrBSft[i]);   // U0.3 [0,7]
        pstShrpReg->u8chrGSft[i]  =  SHARPEN_CLIP3(0, 0x7, pstShrpReg->u8chrGSft[i]);   // U0.3 [0,7]
    }

    // filter
    pstShrpReg->u8lpfSftUD = SHARPEN_CLIP3(3, 0xf, pstShrpReg->u8lpfSftUD);     // U4.0  [3, 15]
    pstShrpReg->u8lpfSftD  = SHARPEN_CLIP3(3, 0xf, pstShrpReg->u8lpfSftD);      // U4.0  [3, 15]
    pstShrpReg->u8hsfSftUD = SHARPEN_CLIP3(3, 0xf, pstShrpReg->u8hsfSftUD);     // U4.0  [3, 15]
    pstShrpReg->u8hsfSftD  = SHARPEN_CLIP3(3, 0xf, pstShrpReg->u8hsfSftD);      // U4.0  [3, 15]
    ...
    //pstShrpReg->u8rangeDirWgt1 = SHARPEN_CLIP3(0, 0x7F,  pstShrpReg->u8rangeDirWgt1);      // U0.7


    pstShrpReg->u8lfGainWgt    = SHARPEN_CLIP3( 0, 0x1f, pstShrpReg->u8lfGainWgt  );
...
static void SharpenCheckDefDynaReg(ISP_SHARPEN_DYNA_REG_CFG_S *pstShrpReg)
{
    ...
    pstDefaultDynaRegCfg = &(pstShrpReg->stDefaultDynaRegCfg);

    pstDefaultDynaRegCfg->u8gainThdSftD   = SHARPEN_CLIP3(0, 0xf, pstDefaultDynaRegCfg->u8gainThdSftD);    // U4.0
    pstDefaultDynaRegCfg->u8gainThdSelUD  = SHARPEN_CLIP3(0, 0x2, pstDefaultDynaRegCfg->u8gainThdSelUD); // U2.0
    pstDefaultDynaRegCfg->u8gainThdSftUD  = SHARPEN_CLIP3(0, 0xf, pstDefaultDynaRegCfg->u8gainThdSftUD); // U4.0

    pstDefaultDynaRegCfg->u8dirVarSft   = SHARPEN_CLIP3(0, 0xC,  pstDefaultDynaRegCfg->u8dirVarSft);         // U4.0

    //Weak Detail Gain
    pstDefaultDynaRegCfg->u8dirRlyThrhigh= SHARPEN_CLIP3(0, 0x7F,  pstDefaultDynaRegCfg->u8dirRlyThrhigh);         // U4.0
    pstDefaultDynaRegCfg->u8dirRlyThrLow= SHARPEN_CLIP3(0, 0x7F,  pstDefaultDynaRegCfg->u8dirRlyThrLow);         // U4.0

    pstDefaultDynaRegCfg->u8shtVarDiffWgt1 = SHARPEN_CLIP3(0, 0x7F, pstDefaultDynaRegCfg->u8shtVarDiffWgt1);   // U0.7
    pstDefaultDynaRegCfg->u8shtVarWgt0     = SHARPEN_CLIP3(0, 0x7F, pstDefaultDynaRegCfg->u8shtVarWgt0);   // U0.7
    pstDefaultDynaRegCfg->u8RmfGainScale    = SHARPEN_CLIP3(0, 0x1F, pstDefaultDynaRegCfg->u8RmfGainScale); // U1.5  [0, 32]
    pstDefaultDynaRegCfg->u8BmfGainScale    = SHARPEN_CLIP3(0, 0x1F, pstDefaultDynaRegCfg->u8BmfGainScale); // U1.5  [0, 32]
    pstDefaultDynaRegCfg->u8selPixWgt       = SHARPEN_CLIP3( 0, 0x1F, pstDefaultDynaRegCfg->u8selPixWgt );// U0.5
...
static void SharpenCheckMpiDynaReg(ISP_SHARPEN_DYNA_REG_CFG_S *pstShrpReg)
{
    ...
    pstMpiDynaRegCfg = &(pstShrpReg->stMpiDynaRegCfg);

    //MPI
    pstMpiDynaRegCfg->u8bEnShtCtrlByVar = SHARPEN_CLIP3(0, 0x1,  pstMpiDynaRegCfg->u8bEnShtCtrlByVar);   // U1.0
    pstMpiDynaRegCfg->u8shtBldRt        = SHARPEN_CLIP3(0, 0xF,  pstMpiDynaRegCfg->u8shtBldRt);          // U4.0
    pstMpiDynaRegCfg->u8oshtAmt         = SHARPEN_CLIP3(0, 0x7F, pstMpiDynaRegCfg->u8oshtAmt  ); // U0.7
    pstMpiDynaRegCfg->u8ushtAmt         = SHARPEN_CLIP3(0, 0x7F, pstMpiDynaRegCfg->u8ushtAmt  ); // U0.7
    pstMpiDynaRegCfg->u8bEnSkinCtrl     = SHARPEN_CLIP3(0,  0x1, pstMpiDynaRegCfg->u8bEnSkinCtrl);   // U1.0
    pstMpiDynaRegCfg->u8skinEdgeWgt[0]  = SHARPEN_CLIP3(0, 0x1F, pstMpiDynaRegCfg->u8skinEdgeWgt[0]);  // U0.5
    pstMpiDynaRegCfg->u8skinEdgeWgt[1]  = SHARPEN_CLIP3(0, 0x1F, pstMpiDynaRegCfg->u8skinEdgeWgt[1]);  // U0.5
    pstMpiDynaRegCfg->u8bEnLumaCtrl     = SHARPEN_CLIP3(0,  0x1, pstMpiDynaRegCfg->u8bEnLumaCtrl);   // U1.0

    pstMpiDynaRegCfg->bEnWeakDetailAdj = SHARPEN_CLIP3(0, 0x1,  pstMpiDynaRegCfg->bEnWeakDetailAdj);   // U1.0
    pstMpiDynaRegCfg->u8WeakDetailGain = SHARPEN_CLIP3(0, 0x7F,  pstMpiDynaRegCfg->u8WeakDetailGain);          // U4.0

    for ( i = 0; i < SHRP_GAIN_LUT_SIZE; i++ )
    {
        pstMpiDynaRegCfg->u16mfGainD[i]  = SHARPEN_CLIP3(0, 0xFFF, pstMpiDynaRegCfg->u16mfGainD[i]);       // U7.5
        pstMpiDynaRegCfg->u16mfGainUD[i] = SHARPEN_CLIP3(0, 0xFFF, pstMpiDynaRegCfg->u16mfGainUD[i]);      // U7.5

        pstMpiDynaRegCfg->u16hfGainD[i]  = SHARPEN_CLIP3(0, 0xFFF, pstMpiDynaRegCfg->u16hfGainD[i]);       // U7.5
        pstMpiDynaRegCfg->u16hfGainUD[i] = SHARPEN_CLIP3(0, 0xFFF, pstMpiDynaRegCfg->u16hfGainUD[i]);      // U7.5
    }

    for ( i = 0; i < ISP_SHARPEN_LUMA_NUM; i++ )
    {
        pstMpiDynaRegCfg->au8LumaWgt[i] = SHARPEN_CLIP3(0, HI_ISP_SHARPEN_LUMAWGT,  pstMpiDynaRegCfg->au8LumaWgt[i]);   // U0.7
    }
    pstMpiDynaRegCfg->bEnDetailCtrl     = SHARPEN_CLIP3(0,  0x1,  pstMpiDynaRegCfg->bEnDetailCtrl);
    pstMpiDynaRegCfg->u8detailOshtAmt   = SHARPEN_CLIP3(0, 0x7F,  pstMpiDynaRegCfg->u8detailOshtAmt);
    pstMpiDynaRegCfg->u8detailUshtAmt   = SHARPEN_CLIP3(0, 0x7F,  pstMpiDynaRegCfg->u8detailUshtAmt);
    pstMpiDynaRegCfg->u8dirDiffSft      = SHARPEN_CLIP3(0, 0x3F,  pstMpiDynaRegCfg->u8dirDiffSft);        // U6.0

    pstMpiDynaRegCfg->u8bEnChrCtrl   = SHARPEN_CLIP3(0,  0x1, pstMpiDynaRegCfg->u8bEnChrCtrl);      // U1.0
    pstMpiDynaRegCfg->u8chrRGain     = SHARPEN_CLIP3(0, HI_ISP_SHARPEN_RGAIN, pstMpiDynaRegCfg->u8chrRGain); // U1.5  [0, 31]
	pstMpiDynaRegCfg->u16chrGGain    = SHARPEN_CLIP3(0, 0x1FF, pstMpiDynaRegCfg->u16chrGGain); // U4.5
    pstMpiDynaRegCfg->u16chrGmfGain  = SHARPEN_CLIP3(0, 0x1FF, pstMpiDynaRegCfg->u16chrGmfGain); // U4.5
    pstMpiDynaRegCfg->u8chrBGain     = SHARPEN_CLIP3(0, HI_ISP_SHARPEN_BGAIN, pstMpiDynaRegCfg->u8chrBGain); // U1.5  [0, 31]
    pstMpiDynaRegCfg->u16oMaxGain    = SHARPEN_CLIP3( 0, 0x7FF, pstMpiDynaRegCfg->u16oMaxGain  );   // U11.0
    pstMpiDynaRegCfg->u16uMaxGain    = SHARPEN_CLIP3( 0, 0x7FF, pstMpiDynaRegCfg->u16uMaxGain  );   // U11.0
}
...
static HI_S32 ISP_Sharpen_GetDefaultRegCfg(VI_PIPE ViPipe, HI_U32 u32ISO)
{
    ...
    else //if (IS_2to1_WDR_MODE(u8WDRMode) || IS_3to1_WDR_MODE(u8WDRMode) || IS_4to1_WDR_MODE(u8WDRMode) || IS_BUILT_IN_WDR_MODE(u8WDRMode))
    {
...
static void SharpenMPI2Reg(ISP_SHARPEN_REG_CFG_S *pstSharpenRegCfg, ISP_SHARPEN_S *pstSharpen, HI_U32 u32ISO)
{
	...
	//Weak Detail Gain
	  ...
	  pstMpiDynaRegCfg->u8WeakDetailGain = pstSharpen->u8WeakDetailGain;
        ...
        if (pstSharpen->u8ShootSupStr  == 0)
        {
            pstMpiDynaRegCfg->u8bEnShtCtrlByVar = 0;
            pstMpiDynaRegCfg->u8shtBldRt        = 0;
            pstMpiDynaRegCfg->u8shtVarThd1     = 0;
        }
        else
        {
            pstMpiDynaRegCfg->u8bEnShtCtrlByVar = 1;
