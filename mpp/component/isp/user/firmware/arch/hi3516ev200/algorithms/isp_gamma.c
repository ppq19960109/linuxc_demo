...
static HI_VOID GammaExtRegsInitialize(VI_PIPE ViPipe)
{
    ...
    //{
    //    hi_ext_system_gamma_inseg_write(ViPipe, i, g_au16NonUniInSegLut[i]);
    //    hi_ext_system_gamma_step_write(ViPipe, i,  g_au8NonUniStepLut[i]);
    //}
...
static HI_S32 GammaReadExtRegs(VI_PIPE ViPipe)
{
    ...
    if (pstGammaCtx->bLutUpdate)
    {
        ...
        //for ( i = 0 ; i < GAMMA_OUTSEG_NUMBER ; i++ )
        //{
        //    pstGammaCtx->au8Step[i]   = hi_ext_system_gamma_step_read(ViPipe, i);
        //    pstGammaCtx->au16InSeg[i] = hi_ext_system_gamma_inseg_read(ViPipe, i);
        //}
    }
                    ...
                    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (pstGammaCtx->bLutUpdate)
    {
        ...
        for (i = 0; i < pstReg->u8CfgNum; i++)
        {
            ...
            //for ( j = 0 ; j < GAMMA_OUTSEG_NUMBER ; j++ )
            //{
            //    pstUsrRegCfg->au8GammaStep[j]   = pstGammaCtx->au8Step[j];
            //    pstUsrRegCfg->au16GammaInSeg[j] = pstGammaCtx->au16InSeg[j];
            //    pstUsrRegCfg->au16GammaPos[j]   = pstGammaCtx->au16Pos[j];
            //}
