...
HI_S32 REGION_MST_StartVi(HI_VOID)
{
    HI_S32             s32Ret;
    VI_DEV             ViDev               = 0;
    VI_PIPE            ViPipe              = 0;
    ...
    *************************************************/
    ...
    if(g_enMastPipeMode == VI_ONLINE_VPSS_ONLINE)
    {
        ...
        stVbConf.astCommPool[0].u32BlkCnt   = 6+((g_u32DisBufLen==0)?3:0);
    }
    else if(g_enMastPipeMode == VI_OFFLINE_VPSS_ONLINE)
    {
    ...
    else if(g_enMastPipeMode == VI_ONLINE_VPSS_OFFLINE)
    {
        u32BlkSize = COMMON_GetPicBufferSize(stSize.u32Width, stSize.u32Height, SAMPLE_PIXEL_FORMAT, DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN);
        stVbConf.astCommPool[1].u64BlkSize  = u32BlkSize;
        stVbConf.astCommPool[1].u32BlkCnt   = 7;
    }
    else if(g_enMastPipeMode == VI_OFFLINE_VPSS_OFFLINE)
    {
        u32BlkSize = VI_GetRawBufferSize(stSize.u32Width, stSize.u32Height, PIXEL_FORMAT_RGB_BAYER_12BPP, COMPRESS_MODE_NONE, DEFAULT_ALIGN);
        stVbConf.astCommPool[0].u64BlkSize  = u32BlkSize;
        stVbConf.astCommPool[0].u32BlkCnt   = 3;
...
HI_S32 REGION_MST_StopVi(HI_VOID)
{
    SAMPLE_COMM_VI_StopVi(&stViConfig);
    return HI_SUCCESS;
}

HI_S32 REGION_MST_StartVpss(HI_VOID)
{
    HI_S32 i;
    HI_S32 s32Ret;
...
HI_S32 REGION_MST_StopVpss(HI_VOID)
{
    SAMPLE_COMM_VPSS_Stop(0, abChnEnable);
    return HI_SUCCESS;
}

HI_S32 REGION_MST_StartVenc(HI_VOID)
{
    HI_S32          s32Ret;
...
HI_S32 REGION_MST_StopVenc(HI_VOID)
{
    SAMPLE_COMM_VENC_Stop(0);
    return HI_SUCCESS;
}
HI_S32 REGION_MST_StartVo(HI_VOID)
{
    HI_S32 s32Ret;
...
HI_S32 REGION_MST_StopVo(HI_VOID)
{
    SAMPLE_COMM_VO_StopVO(&stVoConfig);
    return HI_SUCCESS;
}

HI_VOID REGION_MST_StartGetVencStream(HI_VOID)
{
...
HI_VOID REGION_MST_StopGetVencStream(HI_VOID)
{
    SAMPLE_COMM_VENC_StopGetStream();
    return;
}
HI_VOID REGION_MST_MPP_VI_VPSS_VENC_START(HI_VOID)
{
    REGION_MST_StartVi();
    REGION_MST_StartVpss();
    SAMPLE_COMM_VI_Bind_VPSS(0, 0, 0);

    REGION_MST_StartVenc();
    SAMPLE_COMM_VPSS_Bind_VENC(0, 0, 0);
    REGION_MST_StartGetVencStream();
    return ;
}

HI_VOID REGION_MST_MPP_VI_VPSS_VENC_END(HI_VOID)
{
    REGION_MST_StopGetVencStream();
...
HI_VOID REGION_MST_MPP_VI_VPSS_VO_START(HI_VOID)
{
    REGION_MST_StartVi();
    REGION_MST_StartVpss();
    SAMPLE_COMM_VI_Bind_VPSS(0, 0, 0);
...
HI_VOID REGION_MST_MPP_VI_VPSS_VO_END(HI_VOID)
{
...
HI_S32 SAMPLE_REGION_VI_VPSS_VO(HI_S32 HandleNum,RGN_TYPE_E  enType,MPP_CHN_S *pstChn)
{
    ...
    REGION_MST_MPP_VI_VPSS_VO_START();
...
HI_S32 SAMPLE_REGION_VI_VPSS_VENC(HI_S32 HandleNum,RGN_TYPE_E  enType,MPP_CHN_S *pstChn)
{
    ...
    REGION_MST_MPP_VI_VPSS_VENC_START();
    ...
    REGION_MST_MPP_VI_VPSS_VENC_END();
    SAMPLE_COMM_SYS_Exit();
    return s32Ret;
}
