...
int event_test_init(struct ev_app_t* ev_app) {
    ...
    sin.sin_port = htons(8090);                  /* Port 8080 */

    int fd = socket(AF_INET, SOCK_STREAM, 0);
    struct bufferevent* bev = bufferevent_socket_new(ev_app->base, -1, BEV_OPT_CLOSE_ON_FREE);
    // evutil_make_socket_nonblocking(fd);

    bufferevent_enable(bev, EV_WRITE | EV_READ);
    bufferevent_setcb(bev, bev_read_cb, bev_write_cb, bev_event_cb, ev_app);
