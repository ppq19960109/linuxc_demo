...
scene_task_timer_t *g_pstTimerList = NULL;
void *g_pool = NULL;
int g_iTimerNum = 0;

void *_scene_task_timer_clock(void *arg);
...
*************************************************************/
extern int
scene_task_timer_init(int iTimerNum)
{
	int iRet = 0;
	...
	g_pstTimerList = 
		(scene_task_timer_t *)malloc(iTimerNum * sizeof(scene_task_timer_t));
	if(NULL == g_pstTimerList)
	{
		return -1;
	}

	memset(g_pstTimerList, 0x0, iTimerNum * sizeof(scene_task_timer_t));
	...
	/*添加定时器时钟任务*/
	thread_pool_add_worker(g_pool, _scene_task_timer_clock, NULL, 0);
...
*************************************************************/
extern int
scene_task_timer_create(PF_TIMERFUN pfTimerExecFun, 
				void* pFunArg,
				int iFunArgLen,
...
*************************************************************/
extern int
scene_task_timer_change(int iTimerId,
...
*************************************************************/
extern int
scene_task_timer_dele(int iTimerId)
{
	/*参数校验*/
...
*************************************************************/ 
extern int
scene_task_timer_reset(int iTimerId,
				PF_TIMERFUN pfTimerExecFun, 
				void* pFunArg,
				unsigned long ulTime,
				int iFunExecTime)
...
*************************************************************/ 
void
scene_task_timer_run(int signal)
{
...
{
	/*注册信号*/
	signal(SIGUSR1, &scene_task_timer_run);
